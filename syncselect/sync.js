Sync.SyncSelect = {
	...
	GetName: function (Item) {
		...
		return MainWindow.Common.SyncSelect.NoExt ? s.replace(/\.[^\.]+$/, "") : s;
	},
	...
	SelectFunc: function (name, IsB, x, fn) {
		InvokeUI("Addons.SyncSelect.Clear");
		this.nAll = 0;
		this.nRetry = 0;
		...
		if (this.nRetry) {
			FVa.SelectItem(null, SVSI_DESELECTOTHERS);
			InvokeUI("Addons.SyncSelect.Retry", [name, IsB, ((this.nAll - this.nRetry) / this.nAll * 100).toFixed(0) + "%"]);
			return;
		} else {
			InvokeUI("Addons.SyncSelect.SetTitle", [GetText(name) + " : "+ FVa.FolderItem.Path]);
		}
	},

	SelectMatches: function (b) {
		Sync.SyncSelect.SelectFunc("Select matches", b, "Items", function (ItemA, ItemB) {
			return ItemB;
		});
	},

	SelectUniques: function (b) {
		Sync.SyncSelect.SelectFunc("Select uniques", b, "Items", function (ItemA, ItemB) {
			return !ItemB;
		});
	},

	SelectNewer: function (b) {
		Sync.SyncSelect.SelectFunc("Select newer", b, "Items", function (ItemA, ItemB) {
			return ItemB && Sync.SyncSelect.CompareDate(ItemA, ItemB) > 0;
		});
	},

	SelectDifferent: function (b) {
		Sync.SyncSelect.SelectFunc("Select different", b, "Items", function (ItemA, ItemB, FVa, FVb) {
			return ItemB && (Sync.SyncSelect.CompareDate(ItemA, ItemB) || Sync.SyncSelect.GetSize(FVa, ItemA) != Sync.SyncSelect.GetSize(FVb, ItemB));
		});
	},

	SelectUniquesNewer: function (b) {
		Sync.SyncSelect.SelectFunc("Select uniques and newer", b, "Items", function (ItemA, ItemB) {
			return !ItemB || Sync.SyncSelect.CompareDate(ItemA, ItemB) > 0;
		});
	},

	SelectUniquesDifferent: function (b) {
		Sync.SyncSelect.SelectFunc("Select uniques and different", b, "Items", function (ItemA, ItemB, FVa, FVb) {
			return !ItemB || Sync.SyncSelect.CompareDate(ItemA, ItemB) || Sync.SyncSelect.GetSize(FVa, ItemA) != Sync.SyncSelect.GetSize(FVb, ItemB);
		});
	},

	SelectSelected: function (b) {
		Sync.SyncSelect.SelectFunc("Select selected", b, "SelectedItems", function (ItemA, ItemB) {
			return ItemB;
		});
	}
}
