const Addon_Id = "createdatecolor";
const item = GetAddonElement(Addon_Id);

Sync.CreateDateColor = {
	Color: []
};
try {
	const smhdw = { s: 1000, m: 60000, h: 3600000, d: 86400000, w: 604800000, y: 31536000000 };
	const ado = OpenAdodbFromTextFile(fso.BuildPath(te.Data.DataFolder, "config\\createdatecolor.tsv"));
	...
	while (!ado.EOS) {
		...
		if (ar[0]) {
			...
			Sync.CreateDateColor.Color.push([s, ar[1] ? GetWinColor(ar[1]) : -1]);
		}
	}
	ado.Close();
} catch (e) { }

Sync.CreateDateColor.Color = Sync.CreateDateColor.Color.sort(function (a, b) {
	return b[0] - a[0];
});
...
AddEvent("ItemPrePaint", function (Ctrl, pid, nmcd, vcd, plRes) {
	if (pid) {
		...
		for (let i = Sync.CreateDateColor.Color.length; i--;) {
			const ar = Sync.CreateDateColor.Color[i];
			if (d < ar[0]) {
				if (ar[1] != -1) {
					vcd.clrText = ar[1];
					return S_OK;
