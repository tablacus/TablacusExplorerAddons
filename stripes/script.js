	...
	if (window.Addon == 1) {
		Addons.Stripes =
		{
			...
			Arrange: function (Ctrl, nDog)
			{
				...
				var hWnd = api.FindWindowEx(Ctrl.hwndView, 0, WC_LISTVIEW, null);
		...
		AddEvent("ViewCreated", Addons.Stripes.Arrange);
		...
		for (var i in cFV) {
			if (cFV[i].hwnd) {
				var hWnd = api.FindWindowEx(cFV[i].hwndView, 0, WC_LISTVIEW, null);
				api.SendMessage(hWnd, LVM_SETBKIMAGE, 0, lvbk);
