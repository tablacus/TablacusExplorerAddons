var Addon_Id = "clipboardimage";

if (window.Addon == 1) {
	Addons.ClipboardImage =
	{
		...
		Exec: function (Ctrl, pt)
		{
			...
			if (image && DropTarget) {
				...
				image.Save(fso.BuildPath(temp, file));
				DropTarget.Drop(fso.BuildPath(temp, file), MK_LBUTTON | MK_SHIFT, api.Memory("POINT"), [DROPEFFECT_MOVE | DROPEFFECT_COPY], false, FV);
				return S_OK;
			}
		}
	};
	...
	AddEvent("Command", function (Ctrl, hwnd, msg, wParam, lParam)
	{
		if (Ctrl.Type <= CTRL_EB || Ctrl.Type == CTRL_TV) {
			if ((wParam & 0xfff) + 1 == CommandID_PASTE) {
				return Addons.ClipboardImage.Exec(Ctrl);
			}
		}
	}, true);
	...
	AddEvent("InvokeCommand", function (ContextMenu, fMask, hwnd, Verb, Parameters, Directory, nShow, dwHotKey, hIcon)
	{
		if (Verb + 1 == CommandID_PASTE) {
			return Addons.ClipboardImage.Exec(ContextMenu.FolderView);
		}
	}, true);
		...
		if (/Background|Edit/i.test(Name) && (api.IsClipboardFormatAvailable(2))) {
			...
			for (var i = api.GetMenuItemCount(hMenu); i-- > 0;) {
				...
				if (mii.fState & MFS_DISABLED) {
					...
					if (s && s.indexOf(Addons.ClipboardImage.String) == 0) {
						api.EnableMenuItem(hMenu, i, MF_ENABLED | MF_BYPOSITION);
						ExtraMenuCommand[mii.wID] = Addons.ClipboardImage.Exec;
					}
				}
			}
		}
