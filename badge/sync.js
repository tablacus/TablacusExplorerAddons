...
Sync.Badge = {
	...
	Enum: function (pid, Ctrl, fncb, SessionId) {
		...
		var Badge = Sync.Badge.BadgePath(pid);
	...
	EditPath: function (Ctrl, pt) {
		...
		if (Selected && Selected.Count == 1) {
			try {
				...
				if (path) {
					var Badge = Sync.Badge.DB.Get(path);
	...
	ExecRemoveItems: function (Ctrl, pt) {
		...
		var FV = ar.shift();
		var Badge = Sync.Badge.BadgePath(FV);
		if (Badge) {
	...
	BadgePath: function (Ctrl) {
		var res = Sync.Badge.IsHandle(Ctrl);
		if (res) {
			return res[1];
		}
	},
	...
	Notify: function () {
		...
		for (var i in cFV) {
			...
			if (FV.hwnd) {
				...
				var s = Sync.Badge.BadgePath(FV);
				if (s) {
					var b = false;
					for (var j in Sync.Badge.Changed) {
...
AddEvent("Load", function () {
	...
	Sync.Badge.DB.OnChange = function (n, s, old) {
		...
		Sync.Badge.Redraw[fso.GetParentFolderName(n)] = true;
	...
	var Installed0 = Sync.Badge.DB.Get('%Installed%').toUpperCase();
	var Installed0 = Sync.Badge.Set('%Installed%', "");
	var Installed1 = Sync.Badge.Portable ? fso.GetDriveName(api.GetModuleFileName(null)).toUpperCase() : "";
...
AddEvent("DragEnter", function (Ctrl, dataObj, grfKeyState, pt, pdwEffect) {
	if (Ctrl.Type <= CTRL_EB || Ctrl.Type == CTRL_DT) {
		var Badge = Sync.Badge.BadgePath(Ctrl);
		if (GetNum(Badge)) {
			return S_OK;
		}
	}
...
AddEvent("Menus", function (Ctrl, hMenu, nPos, Selected, SelItem, ContextMenu, Name, pt) {
	if (/Background|Edit/i.test(Name)) {
		var Badge = Sync.Badge.BadgePath(GetFolderView(Ctrl, pt));
		if (GetNum(Badge)) {
...
AddEvent("ChangeNotify", function (Ctrl, pidls) {
	if (Sync.Badge.DB) {
		if (pidls.lEvent & (SHCNE_RENAMEFOLDER | SHCNE_RENAMEITEM)) {
			...
			var s = Sync.Badge.Remove(pidls[0]);
			if (s) {
		...
		if (pidls.lEvent & (SHCNE_CREATE | SHCNE_MKDIR)) {
			var name = fso.GetFileName(api.GetDisplayNameOf(pidls[0], SHGDN_FORADDRESSBAR | SHGDN_FORPARSING | SHGDN_ORIGINAL));
			var Item = Sync.Badge.SyncItem[name];
			if (Item) {
...
AddEvent("ItemPostPaint2", function (Ctrl, pid, nmcd, vcd) {
	...
	if (hList && pid && Sync.Badge.DB) {
		var image = Sync.Badge.Image[Sync.Badge.DB.Get(pid.Path)];
		if (image) {
...
AddEvent("Sort", function (Ctrl) {
	...
	if (/\-?Tablacus\.Badge$/i.test(Ctrl.SortColumn(1))) {
		Sync.Badge.tid[Ctrl.Id] = setTimeout(function () {
			Sync.Badge.Sort(Ctrl, Ctrl.SortColumn(1));
		}, 99);
	}
});
...
for (var i = 6; --i;) {
	...
	if (s = api.PathUnquoteSpaces(ExtractMacro(te, item.getAttribute("Img" + i)))) {
